...
class Actor(nn.Module):
    ...
    Attributes:
        ...
        a_dynamics (SSM): A state-space model used to handle dynamics.
        num_states (int): Number of input states.
        a_term (torch.Tensor or None): The model term of the action.
    ...
    def forward(
        ...
        if dynamics_input_time_window.dim() == 2:
            ...
            dynamics_input_output = self.a_dynamics(dynamics_input)
            self.a_term = dynamics_input_output.squeeze()[-1, :]
            self.d_term = torch.tanh(direction_term) * self.control_action_upper_bound
            action = (self.a_term) * self.d_term
            action = action.squeeze(0)

        elif dynamics_input_time_window.dim() == 3:
            dynamics_input = dynamics_input_time_window
            dynamics_input_output = self.a_dynamics(dynamics_input)
            a_term = dynamics_input_output.squeeze()[:, -1, :]
            d_term = torch.tanh(direction_term) * self.control_action_upper_bound
            action = (a_term) * d_term
            action = action.squeeze(0)
...
class ADController:
    ...
    def train(self, total_episodes=100, episode_length=100, verbose=True):
        ...
        for ep in range(total_episodes):
            ...
            self.dynamics_states_real = torch.cat(
                [
                    self.actor_model.a_dynamics.LRUR.states_last.real.squeeze(),
                ]
            )
    ...
    def save_model_weights(self, filename):
        ...
        state_dict_list = [
            ...
            self.actor_model.a_dynamics.state_dict(),
            self.target_actor.a_dynamics.state_dict(),
        ]
    ...
    def load_model_weight(self, filename):
        ...
        self.actor_model.a_dynamics.load_state_dict(torch.load(filename)[6])
        self.target_actor.a_dynamics.load_state_dict(torch.load(filename)[7])
